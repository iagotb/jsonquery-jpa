...
public class LessThanBuilderTest {
	...
	public void testGet() {
		...
		Assert.assertEquals("child.money < 3000.75", LessThanBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test (expected = NumberFormatException.class)
	public void testGet_EmptyData() {
		rule = new JsonFilter.Rule("and", "money", "lt", ""); 
		Assert.assertEquals("startsWith(child.name,)", LessThanBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test (expected = NullPointerException.class)
	public void testGet_IncorrectOperator() {
		rule = new JsonFilter.Rule("and", "money", "gt", "3000.75"); 
		Assert.assertEquals("startsWith(child.name,Mik)", LessThanBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test (expected = RuntimeException.class)
	public void testGet_IncorrectField() {
		rule = new JsonFilter.Rule("and", "age", "lt", "3000.75"); 
		Assert.assertEquals("startsWith(child.name,Mik)", LessThanBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
