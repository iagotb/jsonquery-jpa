...
public class BeginsWithBuilderTest {
	...
	@Test
	public void testGet() {
		rule = new JsonFilter.Rule("and", "name", "bw", "Mik"); 
		Assert.assertEquals("startsWith(child.name,Mik)", BeginsWithBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test
	public void testGet_EmptyData() {
		rule = new JsonFilter.Rule("and", "name", "bw", ""); 
		Assert.assertEquals("startsWith(child.name,)", BeginsWithBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test (expected = NullPointerException.class)
	public void testGet_IncorrectOperator() {
		rule = new JsonFilter.Rule("and", "name", "cn", "Mik"); 
		Assert.assertEquals("startsWith(child.name,Mik)", BeginsWithBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
	
	@Test (expected = RuntimeException.class)
	public void testGet_IncorrectField() {
		rule = new JsonFilter.Rule("and", "age", "bw", "Mik"); 
		Assert.assertEquals("startsWith(child.name,Mik)", BeginsWithBuilder.get(Child.class, "child", booleanBuilder, rule).toString());
	}
}
